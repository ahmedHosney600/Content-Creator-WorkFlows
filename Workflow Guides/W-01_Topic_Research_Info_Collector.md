...
REVISION MODE:
If revision_count > 0, start from the current revised_research and apply only unfixed issues.
...
```json
{
  ...
  "revised_research": "Complete revised information architecture — full document with all fixes applied"
}
```
...
(If > 0, start from revised_research — do not start over.)

PRIOR CRITIQUE REPORT:
{{#Research_Refinement_Loop.report#}}

CURRENT RESEARCH STATE (use this if revision_count > 0):
{{#Research_Refinement_Loop.revised_research#}}

RESEARCH EXPANSION RESULTS (new findings from this pass):
{{#Research_Expander.text#}}

ORIGINAL ARCHITECTURE (use if this is first pass):
{{#Information_Architecture_Builder.text#}}
