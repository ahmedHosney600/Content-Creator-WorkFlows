...
REVISION MODE:
If this is a revision pass (revision_count > 0), start from the previously revised ideas in
`revised_ideas` and apply only the remaining unfixed issues from `report`. Do not start over.
...
```json
{
  ...
  "revised_ideas": "Complete revised idea list with ALL 10 ideas in the same format — apply ALL critical and warning fixes"
}
```
...
(If > 0, this is a revision pass. Start from the revised ideas below, not from scratch.)

PRIOR CRITIQUE REPORT (apply remaining unfixed issues):
{{#Idea_Refinement_Loop.report#}}

CURRENT IDEAS TO AUDIT (use revised_ideas if revision_count > 0, otherwise use original):
{{#Idea_Refinement_Loop.revised_ideas#}}

ORIGINAL IDEAS (for first pass only):
{{#Idea_Generator.text#}}
...
Audit every idea. Output ONLY the JSON object.
```
...
IDEAS (post-critique — use these as the final version):
{{#Idea_Refinement_Loop.revised_ideas#}}
