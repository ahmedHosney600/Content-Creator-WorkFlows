...
[Node 2: Hook Writer]                   ← Write 3 opening variations, scored against hook science
      │
      ▼
[Node 3: Script Body Writer]            ← Write the complete script with visual notes & cues
      │
      ▼
[Node 4: CTA & Retention Layer Writer]  ← Add open loops, re-hooks, and CTA variants
      │
      ▼
[Node 5: Egyptian Dialect & Voice Authenticity Layer]
  ← Runs ONLY when scriptLanguage = Arabic (Egyptian)
      │
      ▼
[Node 6: Script Refinement]             ← Read full script for flow, spoken language, pacing
      │
      ▼
╔══════════════════════════════════════════════════════════════════╗
...
RESEARCH PACKAGE:
{{#Start.researchPackage#}}

CONTENT TYPE: {{#Start.contentType#}}
SCRIPT LANGUAGE: {{#Start.scriptLanguage#}}
TARGET DURATION: {{#Start.targetDuration#}}
TARGET PLATFORM: {{#Start.targetPlatform#}}
VOICE STYLE: {{#Start.voiceStyle#}}
AUDIENCE LEVEL: {{#Start.audienceLevel#}}
SCRIPT FORMAT: {{#Start.scriptFormat#}}
CTA GOAL: {{#Start.ctaGoal#}}
MANDATORY MENTIONS: {{#Start.mandatoryMentions#}}
REFERENCE STYLE: {{#Start.referenceScripts#}}
SENSITIVE AREAS: {{#Start.sensitiveHandling#}}
...
LANGUAGE: {{#Start.scriptLanguage#}}
MANDATORY MENTIONS: {{#Start.mandatoryMentions#}}
SENSITIVE HANDLING: {{#Start.sensitiveHandling#}}
...
### Node 4: CTA & Retention Layer Writer
...
> Use a condition node before it: `IF {{#Start.scriptLanguage#}} contains "Arabic"` → run this node.
...
DIALECT REGISTER: {{#Start.dialectRegister#}}
CODE-SWITCHING LEVEL: {{#Start.codeSwitchingLevel#}}
...
{{#CTA_Retention_Layer_Writer.text#}}
...
{{#Egyptian_Dialect_Authenticity_Layer.text#}}
...
**Max Iterations**: `2`
**Break Condition**:
- English/other: `{{#Script_Quality_Loop.grade#}} contains "A"`
...
REVISION MODE:
If revision_count > 0, start from revised_script and apply only unfixed issues from report.
...
```json
{
  ...
  "revised_script": "Complete revised script — ALL fixes applied, full document",
...
REVISION COUNT: {{#Script_Quality_Loop.revision_count#}}
(If > 0, start from revised_script, not from scratch.)

PRIOR CRITIQUE REPORT (apply remaining unfixed issues):
{{#Script_Quality_Loop.report#}}

CURRENT SCRIPT STATE (use if revision_count > 0):
{{#Script_Quality_Loop.revised_script#}}

ASSEMBLED SCRIPT (use for first pass):
{{#Script_Refinement.text#}}
...
* **Input Variables**: `llm_output` = `{{#Self_Critique.text#}}`, `language` = `{{#Start.scriptLanguage#}}`
...
|---|---|---|---|
| 1 | `{{#Script_Quality_Loop.grade#}}` | over-write | `{{#Critique_Parser.current_grade#}}` |
| 2 | `{{#Script_Quality_Loop.report#}}` | over-write | `{{#Critique_Parser.current_report#}}` |
| 3 | `{{#Script_Quality_Loop.revised_script#}}` | over-write | `{{#Critique_Parser.current_revised_script#}}` |
| 4 | `{{#Script_Quality_Loop.dialect_score#}}` | over-write | `{{#Critique_Parser.current_dialect_score#}}` |
| 5 | `{{#Script_Quality_Loop.revision_count#}}` | += | `1` |
...
CRITIQUE GRADE: {{#Script_Quality_Loop.grade#}}
CRITIQUE REPORT: {{#Script_Quality_Loop.report#}}
DIALECT SCORE: {{#Script_Quality_Loop.dialect_score#}}
...
* **Node Title**: `End`
* **Node Type**: `end`
* **Output**: `text` = `{{#Final_Script_Package.text#}}`
...
|---|---|---|---|---|
| `Start` | form submission | `Script_Strategy_Planner` | all `{{#Start.*#}}` | Unconditional |
