...
> **Purpose**: Generates storyboard, pacing, retention map, thumbnails, and B-roll concepts with an autonomous critique loop.

This guide provides the complete, authoritative, and 100% production-ready specification for the **Video Pre-Planning Pipeline** (`W-04_Video_Pre-Planning_Pipeline.yml`) in **Dify Workflow** (v0.7.0+ / v1.0+ DAG engine).
...
* **Text-Prompt Only Output**: All image and video concepts are delivered as **text prompts** ready to be pasted into image/video generation models. No external API calls to generation services are made by this pipeline.
...
┌─────────────────────────────────────────────────────────────────────────┐
│                          1. START NODE (Intake)                         │
│             User Input (31 Video Production Parameters)                 │
└────────────────────────────────────┬────────────────────────────────────┘
                                     │
                                     ▼
┌─────────────────────────────────────────────────────────────────────────┐
...
└────────────────────────────────────┬────────────────────────────────────┘
                                     ...
                                     ▼
┌─────────────────────────────────────────────────────────────────────────┐
│                   6. THUMBNAIL IDEATION (LLM Node)                      │
│     Generates 3 concepts (Emotion, Curiosity, Value) + image prompts   │
└────────────────────────────────────┬────────────────────────────────────┘
                                     │
                                     ▼
╔═════════════════════════════════════════════════════════════════════════╗
║          7. LOOP COUNT GUARD (Native Dify Loop Container Node)          ║
...
╚════════════════════════════════════╦════════════════════════════════════╝
                                     ...
                                     ▼
┌─────────────────────────────────────────────────────────────────────────┐
│                   8. B-ROLL CONCEPT STUDIO (LLM Node)                   │
│   Develops 2-3 visual concepts per B-roll section + image/video prompts │
└────────────────────────────────────┬────────────────────────────────────┘
                                     │
                                     ▼
┌─────────────────────────────────────────────────────────────────────────┐
│                   9. QA & FINAL PACKAGE (LLM Node)                      │
│    Executes 9-point QA check & compiles 11-part Editor Deliverable      │
└────────────────────────────────────┬────────────────────────────────────┘
                                     │
                                     ▼
┌─────────────────────────────────────────────────────────────────────────┐
...
> **Why is B-Roll Concept Studio placed AFTER the Loop?**
> The B-Roll Studio depends on the **post-revision storyboard** (`{{#1786755622902.revised_storyboard#}}`), which is the final, critique-approved version of the shot list. Running it before the loop would mean generating B-roll concepts against an unreviewed draft that may be significantly restructured by the Self-Critique engine — wasting computation and producing misaligned concepts. By placing it after the loop exits, it always operates on the best, most stable version of the storyboard.
...
Configure the 31 input variables in the Start node:

| # | Variable Name | Type | UI Label | Options / Allowed Values | Default Value | Required |
|---|---|---|---|---|---|---|
| 1 | `videoTopic` | `paragraph` | What is the video about? | — | `Formula 1 Racing - The Battle of Speed, Courage and Survival.` | **Yes** |
...
| 3 | `coreMessage` | `text-input` | Core message (one sentence) | — | `Formula 1 is not just driving; it is an uncompromising battle of human limits and precision.` | **Yes** |
...
| 28 | `scriptPackage` | `paragraph` | Script Package | — | `Unspecified` | No |
| 29 | `researchPackage` | `paragraph` | Research Package | — | `Unspecified` | No |
| 30 | `availableFootage` | `paragraph` | Available Footage | — | `Unspecified` | No |
| 31 | `creatorProfile` | `paragraph` | Creator Profile (optional) | — | `Unspecified` | No |
...
- script_package, research_package, available_footage, creator_profile
...
- **Script Package**: [Summarize script_package if provided; otherwise 'None provided']
- **Research Package**: [Summarize research_package if provided; otherwise 'None provided']
- **Available Footage**: [Summarize available_footage if provided; otherwise 'None provided']
...
- **Creator Profile**: [Summarize creator_profile if provided; otherwise 'None provided']
...
- If script_package, research_package, or available_footage are provided, summarize their key details in the Assets & Resources section so all downstream nodes can use them.
...
Script Package: {{#1786742809170.scriptPackage#}}
Research Package: {{#1786742809170.researchPackage#}}
Available Footage: {{#1786742809170.availableFootage#}}
Creator Profile: {{#1786742809170.creatorProfile#}}
...
### Node 5: Thumbnail Ideation
* **Node ID**: `W04_Thumbnail_Ideation`
* **Node Title**: `Thumbnail Ideation`
* **Node Type**: `llm`
* **Model Configuration**:
  * **Provider**: `OpenAI-API-compatible`
  * **Model**: `gemini-advanced`
...
#### System Prompt
```
You are an expert YouTube strategist and thumbnail designer. Generate 3 distinct thumbnail concepts based on the narrative structure, retention map, and creative strategy.
...
- Hero image description (visual composition, subject, framing)
- Text overlay (max 5 words)
- Color palette (primary, accent, background)
- Click psychology (why it works)
- Image generation prompt: A detailed, paste-ready text prompt for an image generation model (e.g., Midjourney, Stable Diffusion, Firefly). Include style, lighting, mood, composition, camera angle, lens details, and aspect ratio.
- A/B test recommendation

After the 3 concepts, provide:
- Recommended Shot: Which storyboard shot number is most click-worthy as a standalone thumbnail
- Final A/B Strategy: Which 2 concepts to A/B test first and why
```

#### User Prompt
```
Design 3 thumbnail concepts based on this project.
...
## 4. 🔄 Node 6: Loop Container — Storyboard & Pacing Self-Critique Engine
...
def main(llm_output: str) -> dict:
    ...
    try:
        ...
        # Fail CLOSED to F — never let unparseable auditor output silently pass as a grade of A or B
        return {
            "current_grade":              "F",
...
## 5. 🎬 Node 7: B-Roll Concept Studio

> **Placement Rationale**: B-Roll Studio runs **after** the Loop exits because it consumes `{{#1786755622902.revised_storyboard#}}` — the critique-approved, final shot list. Generating B-roll concepts against an unreviewed draft would waste computation and produce concepts misaligned with shots the Self-Critique may restructure. By placing it post-loop, it always operates on the best, most stable version of the storyboard.

### Node 7: B-Roll Concept Studio
* **Node ID**: `W04_BRoll_Concept_Studio`
* **Node Title**: `B-Roll Concept Studio`
* **Node Type**: `llm`
* **Model Configuration**:
  * **Provider**: `OpenAI-API-compatible`
  * **Model**: `gemini-advanced`
...
#### System Prompt
```
You are an Art Director and visual prompt engineer designing B-roll for a video. Your output is purely textual — no images or videos are generated here. Instead, you produce detailed text prompts and descriptions that a human can paste directly into image or video generation tools (e.g., Midjourney, Stable Diffusion, Runway, Kling, Sora).

For every B-roll section identified in the storyboard, generate 2-3 distinct visual concepts beyond the literal illustration. Each concept should offer a different creative angle or metaphor.

Also produce a B-Roll Style Guide that defines the visual thread connecting all B-roll inserts so the final video reads as a coherent visual whole.
...
#### B-ROLL STYLE GUIDE
- Overall Visual Thread: [The unifying visual aesthetic — e.g., "all inserts use slow-motion, underexposed, high-contrast"]
- Primary Shot Types Used: [e.g., extreme close-up, POV, overhead]
- Camera Movement Style: [e.g., all handheld, all gimbal, mixed]
- Color & Lighting Direction: [e.g., desaturated with warm highlights, neon-lit night scenes]
- Mood Consistency Rule: [One rule all B-roll must follow to stay coherent]

---

#### B-ROLL CONCEPTS TABLE

For each B-roll slot, repeat this block for each concept (2-3 per slot):

**[Slot Label — e.g., "B-Roll Slot: 0:08–0:12 | Supporting Act 1"]**

| Field | Details |
|-------|---------|
| **Timestamp Range** | [start – end from storyboard] |
| **Storyboard Context** | [Brief description of what the main storyboard says is happening here] |
| **Concept Name** | [Short creative title for this concept] |
| **Visual Description** | [What the viewer sees — be cinematic and specific] |
| **Metaphor / Meaning** | [What this visual communicates beyond the literal] |
| **Style Note** | [Lighting, color, texture, pace] |
| **Fulfillment Path** | Film it / Stock footage / Generate image prompt / Generate video prompt |
| **Image Generation Prompt** | [Only if path is "Generate image prompt" — paste-ready prompt for an image model. Include: subject, style, lighting, mood, camera angle, lens, color grade, aspect ratio] |
| **Video Generation Prompt** | [Only if path is "Generate video prompt" — paste-ready prompt for a video model. Include: scene description, camera movement, duration hint, mood, visual style, color grade] |

---

RULES:
- NEVER call any external API or service. Your entire output is text only.
- Image and video generation prompts must be self-contained and detailed enough to use without any additional context.
- Offer at least one "Film it" or "Stock footage" concept per slot alongside any generated-prompt concepts.
- Concepts should span a creative range: one literal, one metaphorical, one abstract/artistic.
```

#### User Prompt
```
Develop B-Roll Concepts and a Style Guide for this storyboard.
...
STORYBOARD (post-revision — final approved version):
{{#1786755622902.revised_storyboard#}}
```

---

## 6. 📦 Final QA & Delivery Nodes

---

### Node 8: QA & Final Package
...
I. B-ROLL CONCEPTS & STYLE GUIDE:
- B-roll concepts align with the visual mood and storyboard
- Image/video generation prompts are detailed and paste-ready
- Fulfillment paths are practical given the available assets
- Style Guide provides a coherent visual thread
...
[Include all 3 thumbnail concepts with their image generation prompts]
...
**9. B-ROLL CONCEPTS & STYLE GUIDE**:
[Include the full B-Roll Style Guide and all concept tables with image/video prompts]

**10. EDITOR'S QUICK REFERENCE CARD**:
...
CRITIQUE GRADE: {{#1786755622902.grade#}}

B-ROLL CONCEPTS & STYLE GUIDE:
{{#W04_BRoll_Concept_Studio.text#}}

Perform the final QA checklist, then compile the complete Final Video Pre-Planning Package with all sections including the JSON export. If the critique grade is "F" (unparseable auditor output on the final pass), add this line at the top of the QA section: "⚠️ Auditor output could not be parsed on the final pass — review the storyboard and pacing manually before handing off to editing."
```

---

### Node 9: Output (End Node)
...
```json
{
  ...
  "scriptPackage": "Unspecified",
  "researchPackage": "Unspecified",
  "availableFootage": "Unspecified",
  "creatorProfile": "Unspecified"
}
```

### Execution Verification Checklist
1. **Brief Builder (Node 1)**: Verifies that all optional unspecified inputs are silently populated with `(Auto)` inferred values. Confirms `scriptPackage`, `researchPackage`, `availableFootage`, and `creatorProfile` are surfaced in the Assets & Resources section of the brief.
...
5. **Thumbnail Ideation (Node 5)**: Produces 3 thumbnail concepts (Emotion, Curiosity, Value), each with a detailed, paste-ready image generation prompt and A/B test recommendation.
6. **Loop Container (Node 6)**:
...
7. **B-Roll Concept Studio (Node 7)**: Consumes the post-revision storyboard. Verifies every slot has 2-3 concepts with the correct fulfillment path and paste-ready image/video generation prompts. Confirms no external API calls are made.
8. **QA & Final Package (Node 8)**: Evaluates the 9 QA dimensions (A through I) and formats the final deliverable with Section 11 containing valid, parsable JSON storyboard data.
9. **End Node (Node 9)**: Delivers full text response without truncation.
