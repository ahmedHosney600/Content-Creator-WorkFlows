...
│     Generates 3 concepts (Emotion, Curiosity, Value) + A/B strategy     │
└────────────────────────────────────┬────────────────────────────────────┘
                                     │
                                     ▼
┌─────────────────────────────────────────────────────────────────────────┐
│             7. STORYBOARD VISUALIZER (Tool/HTTP Request)                │
│    Selective image generation for hook, thumbnails, and B-roll (Opt-in) │
└────────────────────────────────────┬────────────────────────────────────┘
                                     │
                                     ▼
╔═════════════════════════════════════════════════════════════════════════╗
║          8. LOOP COUNT GUARD (Native Dify Loop Container Node)          ║
...
╚════════════════════════════════════┬════════════════════════════════════╝
                                     ...
                                     ▼
┌─────────────────────────────────────────────────────────────────────────┐
│                   9. B-ROLL CONCEPT STUDIO (LLM Node)                   │
│      Develops 2-3 visual concepts per B-roll section + style guide      │
└────────────────────────────────────┬────────────────────────────────────┘
                                     │
                                     ▼
┌─────────────────────────────────────────────────────────────────────────┐
│                   10. QA & FINAL PACKAGE (LLM Node)                     │
...
Configure the 33 input variables in the Start node:
...
| 31 | `enableImageConcepts` | `select` | Enable Image Concepts? | `Yes`, `No` | `No` | No |
| 32 | `imageGenProvider` | `text-input` | Image Gen Provider | — | `Unspecified` | No |
| 33 | `creatorProfile` | `paragraph` | Creator Profile (optional) | — | `Unspecified` | No |
...
### Node 7: Storyboard Visualizer
* **Node ID**: `W04_Storyboard_Visualizer`
* **Node Title**: `Storyboard Visualizer`
* **Node Type**: `http-request` (Tool)
* **Condition**: Runs ONLY if `enableImageConcepts = Yes`

Calls an image-generation model (via `imageGenProvider`) to generate preview images for:
1. The hook shot (first 3 seconds)
2. 3 thumbnail concepts
3. Any B-Roll Concept Studio row tagged `AI-generated image`

*Text description remains the source of truth; image is a visual aid.*


## 4. 🔄 Node 8: Loop Container: Storyboard & Pacing Self-Critique Engine
...
#### System Prompt
```markdown
You are an Art Director designing B-roll for a video.
Generate 2-3 visual concepts beyond the literal illustration for every B-roll section.

Include a B-Roll Style Guide: which shot types the project leans on so all inserts read as one visual thread.

For each concept, specify:
- Timestamp range
- Visual concept
- Style note
- Fulfillment Path: `Film it` / `Stock footage` / `AI-generated image` / `AI-generated video`

For AI-generated video concepts, use standard AI prompt formatting.
```
...
- Does it align with the visual mood?
- Are fulfillment paths practical?
...
**8. THUMBNAIL RECOMMENDATION & CONCEPTS**:
- Recommended Shot: [Which shot # is most click-worthy as a standalone image]
- 3 Thumbnail Concepts: [Emotion, Curiosity, Value]
- Visualizer Previews (if generated)


**9. B-ROLL CONCEPTS & STYLE GUIDE**:
[Include the full B-Roll style guide and concept table]
